...
export async function fetchUserContributions(username: string, start: string, end: string) {
  const [mergedPRs, reviewedPRs, draftPRs, closedPRs, issuesOpened, comments] = await Promise.all([
...
async function getUserComments(username: string, start: string, end: string) {
  ...
  const issueComments = await fetchAllPages(`https://api.github.com/users/${username}/issues/comments`)
  ...
  const prComments = await fetchAllPages(`https://api.github.com/users/${username}/pulls/comments`)
